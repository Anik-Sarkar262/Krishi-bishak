    ...
    const state = {
        scans: [
            { crop: 'Tomato', disease: 'Healthy', date: '2025-12-24', conf: '99%', type: 'good' },
            { crop: 'Potato', disease: 'Early Blight', date: '2025-12-22', conf: '88%', type: 'bad' },
            { crop: 'Corn', disease: 'Healthy', date: '2025-12-20', conf: '96%', type: 'good' },
        ],
        tasks: [
            { id: 1, text: 'Apply Fungicide on Potato Row A', date: '2025-12-25', done: false, tag: 'Urgent' },
            { id: 2, text: 'Irrigate Tomato Sector 2', date: '2025-12-26', done: false, tag: 'Routine' }
        ],
        // community: [
    ...
    //                     <button class="vote-btn agree" onclick="vote(this)">👍 Agree <span>${c.votes}</span></button>
    //                     <button class="vote-btn disagree" onclick="vote(this)">👎 Disagree</button>
    //                 </div>
    //             </div>
    //         </div>
    //     `).join('');
    ...
    function vote(btn) {
        ...
        if(btn.classList.contains('active')) {
            ...
            if(state.smsEnabled && Math.random() > 0.5) {
                setTimeout(() => showToast("📲 SMS Sent: Neighbor posted new case"), 1000);
            }
        }
    }
